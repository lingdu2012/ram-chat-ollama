...
MessageList.value= JSON.parse(localStorage.getItem('chatList') || '[]') as Array<{id: string;model:string; message: string}>;

for(let item of MessageList.value){
  if(item.id==messageId.value){
      restModel(item.model);
      for(let content of JSON.parse(item.message)){
        addChatContextItem(content);
      }
  }
}
